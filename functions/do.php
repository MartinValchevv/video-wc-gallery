						...
						<li class="ui-state video_id_<?php echo esc_attr($key) ?>" data-position="<?php echo esc_attr($position_counter) ?>">
							<div class="video-player">
								...
								<div>
									...
									<input type="hidden" class="caption_url" name="video_url[<?php echo esc_attr($key) ?>][caption_url]" value="<?php echo isset($video['caption_url']) ? esc_url($video['caption_url']) : ''; ?>" />
									<input type="hidden" class="caption_lang" name="video_url[<?php echo esc_attr($key) ?>][caption_lang]" value="<?php echo isset($video['caption_lang']) ? esc_attr($video['caption_lang']) : 'en'; ?>" />
								</div>

								<button data-video-id="<?php echo esc_attr($key) ?>" type="button" class="button delete_video_btn" title="Delete">X</button>
							</div>
							<div class="caption-controls">
								<button data-video-id="<?php echo esc_attr($key) ?>" type="button" class="button add_caption_btn" title="<?php echo esc_attr__('Add Caption', 'video-wc-gallery') ?>"><?php echo isset($video['caption_url']) ? esc_html__('Change Caption', 'video-wc-gallery') : esc_html__('Add Caption', 'video-wc-gallery') ?></button>
								<?php if (isset($video['caption_url'])) : ?>
									<select class="caption-language" style="display: inline;" data-video-id="<?php echo esc_attr($key) ?>" name="video_url[<?php echo esc_attr($key) ?>][caption_lang]">
										<option value="en" <?php selected(isset($video['caption_lang']) ? $video['caption_lang'] : 'en', 'en'); ?>><?php echo esc_html__('English', 'video-wc-gallery') ?></option>
										<option value="es" <?php selected(isset($video['caption_lang']) ? $video['caption_lang'] : 'en', 'es'); ?>><?php echo esc_html__('Spanish', 'video-wc-gallery') ?></option>
										<option value="fr" <?php selected(isset($video['caption_lang']) ? $video['caption_lang'] : 'en', 'fr'); ?>><?php echo esc_html__('French', 'video-wc-gallery') ?></option>
										<option value="de" <?php selected(isset($video['caption_lang']) ? $video['caption_lang'] : 'en', 'de'); ?>><?php echo esc_html__('German', 'video-wc-gallery') ?></option>
										<option value="it" <?php selected(isset($video['caption_lang']) ? $video['caption_lang'] : 'en', 'it'); ?>><?php echo esc_html__('Italian', 'video-wc-gallery') ?></option>
										<option value="pt" <?php selected(isset($video['caption_lang']) ? $video['caption_lang'] : 'en', 'pt'); ?>><?php echo esc_html__('Portuguese', 'video-wc-gallery') ?></option>
										<option value="ru" <?php selected(isset($video['caption_lang']) ? $video['caption_lang'] : 'en', 'ru'); ?>><?php echo esc_html__('Russian', 'video-wc-gallery') ?></option>
										<option value="zh" <?php selected(isset($video['caption_lang']) ? $video['caption_lang'] : 'en', 'zh'); ?>><?php echo esc_html__('Chinese', 'video-wc-gallery') ?></option>
										<option value="ja" <?php selected(isset($video['caption_lang']) ? $video['caption_lang'] : 'en', 'ja'); ?>><?php echo esc_html__('Japanese', 'video-wc-gallery') ?></option>
										<option value="ko" <?php selected(isset($video['caption_lang']) ? $video['caption_lang'] : 'en', 'ko'); ?>><?php echo esc_html__('Korean', 'video-wc-gallery') ?></option>
									</select>
								<?php endif; ?>
							</div>
						</li>
	...
	if (isset($_POST['video_url'])) {
		...
		foreach ($_POST['video_url'] as $key => $attachment) {
			...
			$sanitized_attachment = array(
				...
				'caption_url' => isset($attachment['caption_url']) ? wp_kses_post($attachment['caption_url']) : '',
			);
...
?>
	<script type="text/javascript">
		jQuery(document).ready(function($) {
                                ...
                                <div class="video-player">
                                    <video width="230" height="200" controls preload="auto">
                                        <source src="${attachment.url}" type="video/webm">
																		...
																		<input type="hidden" class="caption_url" name="video_url[${attachment.id}][caption_url]" value=""/>
																		<input type="hidden" class="caption_lang" name="video_url[${attachment.id}][caption_lang]" value="en"/>
																		
																		<button data-video-id="${attachment.id}" type="button" class="button delete_video_btn" title="Delete">X</button>
																</div>
																<div class="caption-controls">
                                    <button data-video-id="${attachment.id}" type="button" class="button add_caption_btn" title="<?php echo esc_attr__('Add Caption', 'video-wc-gallery') ?>"><?php echo esc_html__('Add Caption', 'video-wc-gallery') ?></button>
                                    <select class="caption-language" data-video-id="${attachment.id}" name="video_url[${attachment.id}][caption_lang]">
                                        <option value="en"><?php echo esc_html__('English', 'video-wc-gallery') ?></option>
                                        <option value="es"><?php echo esc_html__('Spanish', 'video-wc-gallery') ?></option>
                                        <option value="fr"><?php echo esc_html__('French', 'video-wc-gallery') ?></option>
                                        <option value="de"><?php echo esc_html__('German', 'video-wc-gallery') ?></option>
                                        <option value="it"><?php echo esc_html__('Italian', 'video-wc-gallery') ?></option>
                                        <option value="pt"><?php echo esc_html__('Portuguese', 'video-wc-gallery') ?></option>
                                        <option value="ru"><?php echo esc_html__('Russian', 'video-wc-gallery') ?></option>
                                        <option value="zh"><?php echo esc_html__('Chinese', 'video-wc-gallery') ?></option>
                                        <option value="ja"><?php echo esc_html__('Japanese', 'video-wc-gallery') ?></option>
                                        <option value="ko"><?php echo esc_html__('Korean', 'video-wc-gallery') ?></option>
                                    </select>
                                </div>
                            </li>
                        `)
			...
			$(document).on('click', '.add_caption_btn', function(event) {
				event.preventDefault();
				var videoId = $(this).data('video-id');
				var caption_frame = wp.media.frames.caption_frame = wp.media({
					title: '<?php echo esc_html__('Select Caption File', 'video-wc-gallery') ?>',
					button: {
						text: '<?php echo esc_html__('Select Caption File', 'video-wc-gallery') ?>'
					},
					library: {
				...
				caption_frame.on('select', function() {
					var attachment = caption_frame.state().get('selection').first().toJSON();
					$(`.video_id_${videoId} .caption_url`).val(attachment.url);
					$(`.video_id_${videoId} .add_caption_btn`).text('<?php echo esc_html__('Change Caption', 'video-wc-gallery') ?>');
	...
	<style>
		...
		#vwg_video_tab_content .ui-state .delete_video_btn {
			position: absolute;
			top: -10px;
			right: -10px;
			border-radius: 50%;
		...
		#vwg_video_tab_content .caption-controls {
			margin: 0.5rem auto;
			position: relative;
			display: flex;
			gap: 0.75rem;
		...
		#vwg_video_tab_content .caption-controls .add_caption_btn {
			border-radius: 3px;
			flex: 0 0 auto;
		}

		#vwg_video_tab_content .caption-controls .caption-language {
			flex: 1 1 auto;
			min-width: 100px;
			display: none;
		}
...
function vwg_add_custom_style_and_scripts_product_page()
{
	...
	?>
		<style>
			...
			.woocommerce div.product div.images .flex-control-thumbs li .vwg-video-wrapper:hover,
			.woocommerce div.product div.images .flex-control-thumbs li .vwg-video-wrapper.flex-active {
				opacity: 1;
			}
			...
			<?php if (isset($adaptSettings) && $adaptSettings == 1) : ?>.woocommerce-product-gallery__image .woocommerce-product-gallery__vwg_video video {
				object-fit: cover;
			}
			...
			<div data-thumb="<?= esc_url($video['video_thumb_url']) ?>"
				...
				<a href="<?= esc_url($video['video_url']) ?>" class="woocommerce-product-gallery__vwg_video">
					<video id="vwg_video_js_<?= esc_attr($countVideo) ?>" class="video-js <?= esc_attr($adaptClass) ?> vwg_video_js" width="<?= esc_attr($width) ?>" height="<?= esc_attr($height) ?>" preload="auto" <?= esc_attr($controls) ?> <?= esc_attr($autoplay) ?> <?= esc_attr($loop) ?> <?= esc_attr($muted) ?> playsinline data-setup="{}" poster="<?= esc_url($video['video_thumb_url']) ?>">
						...
						<?php if (!empty($video['caption_url'])) :
							$lang = isset($video['caption_lang']) ? $video['caption_lang'] : 'en';
							$lang_name = array(
								'en' => __('English', 'video-wc-gallery'),
								'es' => __('Spanish', 'video-wc-gallery'),
								'fr' => __('French', 'video-wc-gallery'),
								'de' => __('German', 'video-wc-gallery'),
								'it' => __('Italian', 'video-wc-gallery'),
								'pt' => __('Portuguese', 'video-wc-gallery'),
								'ru' => __('Russian', 'video-wc-gallery'),
								'zh' => __('Chinese', 'video-wc-gallery'),
								'ja' => __('Japanese', 'video-wc-gallery'),
								'ko' => __('Korean', 'video-wc-gallery')
							);
						?>
							<track kind="captions" src="<?= esc_url($video['caption_url']) ?>" srclang="<?= esc_attr($lang) ?>" label="<?= esc_attr($lang_name[$lang]) ?>" default>
						<?php endif; ?>
					</video>
				</a>
			</div>
